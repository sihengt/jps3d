...
vec_Vecf<Dim> DMPlanner<Dim>::removeCornerPts(const vec_Vecf<Dim> &path)
{
    ...
    for (unsigned int i = 1; i < path.size() - 1; i++)
    {
        ...
        {
            cost1 = cost3;
            // printf("skip point [%d]: [%f, %f], cost1: %f, cost2: %f, cost3:
    ...
    if (Dim == 2)
    {
        for (n(0) = -rn; n(0) <= rn; n(0)++)
        {
            for (n(1) = -rn; n(1) <= rn; n(1)++)
            {
                ...
                double h =
                    h_max *
                    std::pow((1 - (double)std::hypot(n(0), n(1)) / rn), pow);
                if (h > 1e-3)
        ...
        for (n(0) = -rn; n(0) <= rn; n(0)++)
        {
            for (n(1) = -rn; n(1) <= rn; n(1)++)
            {
                for (n(2) = -hn; n(2) <= hn; n(2)++)
                {
                    ...
                    double h =
                        h_max *
                        std::pow((1 - (double)std::hypot(n(0), n(1)) / rn) *
                                 ...
                                 const vec_Vecf<Dim> &path)
{
    if (planner_verbose_)
    {
        ...
        printf("cweight: %f\n", cweight_);
        printf("pow: %d\n", pow_);
        std::cout << "search_radius: " << search_radius_.transpose()
                  << std::endl;
    ...
    const vec_Vecf<Dim> &prior_path, int max_iteration)
{
    ...
    {
        this->search_region_ = this->setPath(path, this->search_radius_,
                                             false); // sparse input path
