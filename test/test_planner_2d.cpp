...
int main(int argc, char **argv)
{
    ...
    bool valid_jps = planner_ptr->plan(
        start, goal, 1, true); // Plan from start to goal using JPS
    ...
    if (valid_astar)
    {
        ...
        for (auto pt : path)
            line.push_back(point_2d(pt(0), pt(1)));
        mapper.add(line);
        mapper.map(line, "opacity:0.4;fill:none;stroke:rgb(1,212,0);stroke-"
                         "width:5"); // Green
    }
    ...
    mapper.text(point_2d(origin_x + range_x - 5.5, origin_y + 1.2),
                "astar path", "fill-opacity:1.0;fill:rgb(0,0,0);");
