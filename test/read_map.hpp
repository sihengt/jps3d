...
public:
    MapReader(const std::string &file, bool verbose = false)
    {
        try
        {
            ...
            if (!config[0]["start"] || !config[1]["goal"] ||
                !config[2]["origin"] || !config[3]["dim"] ||
                !config[4]["resolution"] || !config[5]["data"])
            {
            ...
            const std::vector<int> &dim_vec =
                config[3]["dim"].as<std::vector<int>>();
            ...
            const std::vector<int> &data =
                config[5]["data"].as<std::vector<int>>();
